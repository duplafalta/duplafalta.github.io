...
if ($_GET['song']) {
	print_r($stream['info']['song']);
} else if ($_GET['artist']) {
	print_r($stream['info']['artist']);
} else {
	print_r($stream['info']['Currently playing']);
}
...
function getStreamInfo(){
	...
	$re  = "/<table\\s[^>]*class=\\\"yellowkeys\\\"><tbody>(.*)<\\/tbody><\\/table>/isU"; 
	if (preg_match($re, $str, $matches)) {
		$stream['info']['status'] = 'ON AIR';
		foreach (array_slice(explode('<tr>', end($matches)), 1) as $row) {
			$res_row = strip_tags($row);
			$pieces = explode(":", $res_row);
			$stream['info'][$pieces[0]] = $pieces[1];
		}
		if (isset($stream['info']['Currently playing'])) {
			$song_info = explode(" - ", $stream['info']['Currently playing']);
			$stream['info']['artist'] = $song_info[0];
			$stream['info']['song'] = $song_info[1];
		}
	} else {
